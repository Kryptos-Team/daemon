...
fn_start_tmux() {
  ...
  tmux new-session -d -x "${session_width}" -y "${session_height}" -s "${self_name}" "${gui_cmd} -conf=${daemon_config_file}" 2>"${root_dir}/.${self_name}-tmux-error.tmp"
...
if [ "${short_name}" == "BTC" ]; then
  gui_cmd="${daemon_bin_dir}/bitcoin-qt"
elif [ "${short_name}" == "DOGE" ]; then
  gui_cmd="${daemon_bin_dir}/dogecoin-qt"
elif [ "${short_name}" == "LTC" ]; then
  gui_cmd="${daemon_bin_dir}/litecoin-qt"
elif [ "${short_name}" == "PPC" ]; then
  gui_cmd="${daemon_bin_dir}/peercoin-qt"
elif [ "${short_name}" == "XPM" ]; then
  gui_cmd="${daemon_bin_dir}/primecoin-qt"
else
